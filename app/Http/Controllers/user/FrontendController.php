...
class FrontendController extends Controller
{
    ...
    public function sendSurvey(Request $request)
    {
            ...
            foreach ($emails as $email) {
                ...
                foreach ($separate_emails as $semail){
                    ...
                    if($isSurveyEmailExists){
                        ...
                        $hash = Crypt::encryptString(json_encode(array('company_id'=>$cId, 'survey_id'=>$input['survey'])));
        ...
        $msgSuccess = '';
        if($success == true){
           $msgSuccess = '<ul><li>'.trans('messages.survey_sent_to').'</li></ul>';
        }
        if($success == false){
            if($error == true){
               $sendedmail=mb_substr($already_send_mail, 0,-1);
               $msgError = '<ul><li>'.trans('messages.already_survey_sent_to').'.</li></ul>';
            }
        }
    ...
    public function takeSurvey(Request $request)
    {
        ...
        if(isset($input['proceed'])) {
            ...
            $request->session()->put('stakeholder_email', $request->email);
            $hash = Crypt::encryptString(json_encode(array('company_id'=>Auth::user()->company_id, 'survey_id'=>$input['survey'])));
    ...
    {
        $locale = LaravelLocalization::getCurrentLocale();
        $surveyData = json_decode(Crypt::decryptString($hash));
        ...
        $request->session()->put('stakeholder_company_id', $surveyData->company_id);
        $request->session()->put('stakeholder_survey_id', $surveyData->survey_id);
        return view('user.stakeholder-welcome', compact('email', 'locale'));
    }

    public function stakeholderWelcomeContinue(Request $request){

        $this->validate(request(), [
            'email' => [
                'required',
                'email',
                Rule::unique('survey_entries')->where(function ($query) use($request) {
                    $query->where('email', $request->email);
                    $query->where('survey_id',$request->session()->get('stakeholder_survey_id'));
                })
            ],
        ]);
        ...
        $survey = Survey::select('fiscal_entry')->find($survey_id);
        $fiscal_entry = explode(' / ', $survey->fiscal_entry);

        $view = view('user.stakeholder-welcome-continue', array('company'=>$company->name,'fiscal_year'=>$fiscal_entry[0]))->render();
    ...
    public function stakeholder(Request $request)
    {
        ...
        $hash = Crypt::encryptString(json_encode(array('company_id'=>$company_id, 'survey_id'=>$survey_id)));
