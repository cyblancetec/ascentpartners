...
class FrontendController extends Controller
{
    ...
    public function sendSurvey(Request $request)
    {
        ...
        if(isset($input['proceed'])) {
            ...
            $subject = str_replace('[FISCAL_YEAR]', $company->fiscal_year, $subject);
            ...
            foreach ($emails as $email) {
                $isSurveyEmailExists = SurveyEntryHelper::surveyEmailExists($email, $input['survey']);
                if($isSurveyEmailExists){

                    $saveData = array('company_id'=>$cId, 'user_id'=>Auth::user()->id, 'survey_id'=>$input['survey'], 'email'=>$email);
                    $hash = Crypt::encryptString(json_encode(array('company_id'=>$cId, 'survey_id'=>$input['survey'], 'email'=>$email)));
                    ...
                    mail($email,$subject,$content,$headers,'-finfo@ascentpartners.com');
                    $success = true;
                    $msgSuccess .= '<li>'.trans('messages.survey_sent_to').' "'.$email.'".</li>';
                }else{
                    $error = true;
                    $msgError .= '<li>'.trans('messages.already_survey_sent_to').' "'.$email.'".</li>';
                }
            }
            $msgError .= '</ul>';
            $msgSuccess .= '</ul>';
    ...
    public function stakeholderWelcome(Request $request, $hash)
    {
        ...
        return view('user.stakeholder-welcome', array('hash'=>$hash));
    }
    ...
    public function stakeholder(Request $request)
    {
        ...
        return view('user.stakeholder', array('stakeholders' => $stakeholders, 'hash'=>$hash));
    }
    ...
    public function stakeholderInstanceSummary(Request $request){
        ...
        $survey_summary = DB::select($sql);
        $surveys = Survey::select('id','title')->where('company_id', Auth::user()->company_id)->orderBy('title')->get();
        
        return view('user.stakeholder-instance-summary', array('surveys' => $surveys, 'survey_summary' => $survey_summary[0], 'survey_id' => $survey_id));
    }
